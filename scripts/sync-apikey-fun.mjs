...
function pickGroup(vendor, enableGroups, groupRatio) {
  const enabled = (Array.isArray(enableGroups) ? enableGroups : []).filter(group => groupRatio.has(group));
  ...
  return enabled.filter(group => !EXCLUDED_GROUPS.test(group)).sort((a, b) => groupRatio.get(a) - groupRatio.get(b))[0] || null;
}
function pricingFor(row, groupMultiplier) {
  ...
  return { input: +input.toFixed(8), output: +output.toFixed(8), cached_input: cachedInput == null ? null : +cachedInput.toFixed(8) };
}
...
function snippet(text, index, before = 700, after = 1700) {
  return text.slice(Math.max(0, index - before), Math.min(text.length, index + after)).replace(/\s+/g, ' ');
}
function logInteresting(label, text) {
  const endpoints = new Set(
    [...text.matchAll(/["'`]((?:https?:\/\/[^"'`\s)]+|\/api\/v1\/[^"'`\s)]*|\/(?:pricing|groups|public|models)[^"'`\s)]*))["'`]/gi)]
      .map(m => m[1])
  );
  console.log(`! ${label} endpoint-candidates=${JSON.stringify([...endpoints].slice(0, 100))}`);
  for (const needle of ['pricing', '/api/v1/', 'model_pricing', 'rate_multiplier', 'input_price', 'output_price', 'cache', 'groups', 'models']) {
    const lower = text.toLowerCase();
    let pos = 0;
    let count = 0;
    while (count < 4) {
      const index = lower.indexOf(needle.toLowerCase(), pos);
      if (index < 0) break;
      console.log(`! ${label}[${needle}]=${snippet(text, index)}`);
...
async function inspectPricingPage(errors) {
  const pageResponse = await fetch(PRICE_URL, { redirect: 'follow', headers: { ...requestHeaders, accept: 'text/html,application/xhtml+xml' } });
  const html = await pageResponse.text();
  const scripts = [...html.matchAll(/<script[^>]+src=["']([^"']+)["']/gi)].map(m => m[1]);
  console.log(`! pricing page: HTTP ${pageResponse.status}, scripts=${JSON.stringify(scripts)}`);

  for (const scriptSrc of scripts) {
    const scriptUrl = new URL(scriptSrc, PRICE_URL);
    ...
    console.log(`! main-bundle=${scriptUrl.href}, length=${js.length}`);

    const chunks = new Set(
      [...js.matchAll(/["'](assets\/(?:PricingView|dashboardPricing|groups|modelCatalog)[^"']+\.js)["']/g)].map(m => m[1])
    );
    console.log(`! pricing-chunks=${JSON.stringify([...chunks])}`);

    for (const chunk of chunks) {
      const chunkUrl = new URL(`/${chunk}`, PRICE_URL);
      const chunkResponse = await fetch(chunkUrl, { redirect: 'follow', headers: { ...requestHeaders, accept: '*/*' } });
      const chunkText = await chunkResponse.text();
      console.log(`! chunk=${chunkUrl.href}, HTTP ${chunkResponse.status}, length=${chunkText.length}`);
      if (chunkResponse.ok) logInteresting(chunk, chunkText);
    }
  }
...
async function fetchPricingPayload() {
  ...
  for (const apiUrl of API_URLS) {
    ...
    if (!response.ok) { errors.push(`${apiUrl} -> HTTP ${response.status}`); continue; }
    const payload = await response.json();
    if (!payload || payload.success === false || !Array.isArray(payload.data)) { errors.push(`${apiUrl} -> invalid payload`); continue; }
...
for (const model of canonicalModels) {
  ...
  if (!group) { console.warn(`! ${model.id} 未找到明确公开分组，保留现有价格`); continue; }
  ...
  if (!current) {
    ...
    provider.models.push(next); currentMap.set(model.id, next); changed = true;
    changes.push(`${model.id}: 新增 ${price.input}/${price.output} (${group})`); continue;
  }
  ...
  if (priceChanged || current.note !== note) {
    current.input = price.input; current.output = price.output;
    if (price.cached_input == null) delete current.cached_input; else current.cached_input = price.cached_input;
    current.note = note; changed = true; changes.push(`${model.id}: 更新为 ${price.input}/${price.output}/${price.cached_input ?? '-'} (${group})`);
  }
}
...
console.log(changed ? `✓ APIKEY.FUN 价格已更新：${changes.join('; ')}；同步时间 ${updatedAtTime}` : `✓ APIKEY.FUN 已成功校验 ${found} 个模型，价格无变化；同步时间 ${updatedAtTime}`);
